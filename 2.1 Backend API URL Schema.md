...
- **`GET` | Trigger Session Generation:** Manually triggers the background job to generate upcoming game sessions.

**Manually Generate Upcoming Game Sessions**
...
- This endpoint likely requires specific user privileges (e.g., Host or Admin) for security, although the exact requirement isn't specified in the provided code.
- Requires `initData` in request headers for authentication.

**Endpoint:**
```
GET {{baseUrl}}/generate_session/
```
...
- `401 Unauthorized`: Authentication failed (invalid `initData`).
- `403 Forbidden`: User may lack necessary permissions (e.g., not a Host/Admin - *exact role unconfirmed*).
- `500 Internal Server Error`: An error occurred during job execution.

**Response Body:**
| Field | Type | Description |
| --- | --- | --- |
| `message` | string | Confirmation message indicating the job was triggered. |
| `error` | string | Error message if the trigger failed. |
...
// Response (200 OK)
{
    "message": "Session generation job triggered successfully."
}
```
---
